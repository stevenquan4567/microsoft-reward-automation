# ⚡ Microsoft Reward Automation (`v2.2.1`)

🌐 **Languages / 语言 / 언어 / 言語 / Idiomas**:  
[🇻🇳 Tiếng Việt](../README.md) | [🇬🇧 English](README_EN.md) | [🇨🇳 简体中文](README_ZH.md) | [🇰🇷 한국어](README_KO.md) | [🇯🇵 日本語](README_JA.md) | [🇪🇸 Español](README_ES.md) | [🇫🇷 Français](README_FR.md) | [🇩🇪 Deutsch](README_DE.md) | [🇮🇹 Italiano](README_IT.md) | [🇧🇷 Português](README_PT.md) | [🇷🇺 Русский](README_RU.md) | [🇮🇳 हिंदी](README_HI.md)

---

> Bing Desktop खोज पर Microsoft Rewards स्वचालन के लिए स्मार्ट ब्राउज़र एक्सटेंशन। इसमें एंटी-बॉट मानव व्यवहार सिमुलेशन, अनूठे कीवर्ड की प्रक्रियात्मक उत्पत्ति, MV3 बैकग्राउंड दैनिक शेड्यूलिंग और ऑटो-अपडेट की सुविधा है।

[![Version](https://img.shields.io/badge/Version-v2.2.1-blue?style=for-the-badge)](#)
[![Manifest](https://img.shields.io/badge/Manifest-V3-00F2FE?style=for-the-badge)](#)
[![Ko-fi](https://img.shields.io/badge/Donate-Ko--fi-FF5E5B?style=for-the-badge&logo=ko-fi&logoColor=white)](https://ko-fi.com/stevenquan45)
[![License](https://img.shields.io/badge/License-MIT-green?style=for-the-badge)](#)

---

## 🌟 प्रमुख विशेषताएं (संस्करण 2.1.0)

### 🖥️ स्वचालित डेस्कटॉप खोज
* **पूर्ण स्वचालन**: Bing Desktop पर अपनी दैनिक निर्धारित खोजें स्वचालित रूप से चलाएं (डिफ़ॉल्ट: 30 खोजें ~90 रिवॉर्ड अंक)।
* **स्मार्ट टैब प्रबंधन**: बैकग्राउंड टैब खोलता है और पूरा होने पर स्वचालित रूप से बंद कर देता है।
* **रियल-टाइम पॉइंट कैलकुलेटर**: पॉपअप इंटरफ़ेस में अर्जित अनुमानित अंक दिखाता है (+3 अंक प्रति खोज)।

### 🛡️ Bing Anti-Bot Humanizer Pro
* **यथार्थवादी मानव व्यवहार सिमुलेशन**:
  * मल्टी-स्टेज स्मूथ स्क्रॉलिंग।
  * प्राकृतिक माउस गति और Bing परिणाम/टैब पर होवर।
  * यादृच्छिक टेक्स्ट चयन।
  * बॉट डिटेक्शन से बचने के लिए समायोज्य विलंब (Min/Max Delay)।

---

## 🌐 योगदान और अनुवाद (Translations & Contributions)

क्या आप एक नई भाषा जोड़ना चाहते हैं? कृपया हमारे [TRANSLATIONS.md](../TRANSLATIONS.md) और [CONTRIBUTING.md](../CONTRIBUTING.md) गाइड को पढ़ें!

---

## ⚖️ लाइसेंस

[MIT License](../LICENSE) के तहत प्रकाशित।
